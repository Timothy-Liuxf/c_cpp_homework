...
int main() {
...

	{
		...
		if (days % 7 <= 4) theFirstDay = 4 - days % 7;
		else theFirstDay = 4 - days % 7 + 7;	//This can calculate January 1st is Monday.
	}
	...
	for (int i = 1; i <= 12; ++i) {
		...
		case 11:
			...
			break;
		case 2:
			if (year % 400 == 0 || year % 4 == 0 && year % 100) days += 29;
			else days += 28;
			break;
		default:
			printf("Error!");
	...
	for (int i = 1; i <= 2019; ++i) {
		...
		for (int j = 1; j <= 12; ++j) {
			...
			case 11:
				...
				break;
			case 2:
				if (i % 400 == 0 || i % 4 == 0 && i % 100) days += 29;
				else days += 28;
				break;
			}
			days += 13;
			if ((lastOfLastYear + days) % 7 == 5) ++flag;
			days -= 13;
		}
